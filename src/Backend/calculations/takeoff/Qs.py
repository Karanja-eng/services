...
@router.post("/api/calculate/stairs", response_model=CalculationResponse)
async def calculate_stairs(request: StairsRequest):
    try:
        ...
        formwork_area = (request.length * 2 + request.width * 2) * (
...
@router.post("/api/calculate/foundation", response_model=CalculationResponse)
...
@router.post("/api/calculate/superstructure", response_model=CalculationResponse)
async def calculate_superstructure(request: SuperstructureRequest):
    try:
        ...
        column_volume = (
            request.number_floors
            * request.storey_height
            * 0.02
            * total_floor_area  # 2% estimate
        )
...
@router.post("/api/calculate/manholes", response_model=CalculationResponse)
async def calculate_manholes(request: ManholesRequest):
    try:
        ...
        excavation_volume = (
            math.pi
            * (excavation_diameter / 2) ** 2
            * (request.depth + base_thickness)
            * request.number_manholes
        )
        ...
        summary = {
            ...
            "reinforcement_weight": round(reinforcement_weight, 2),
            "cover_area": round(
                math.pi * (excavation_diameter / 2) ** 2 * request.number_manholes, 2
            ),
        }
...
@router.post("/api/calculate/pavements", response_model=CalculationResponse)
...
@router.post("/api/calculate/retaining_walls", response_model=CalculationResponse)
async def calculate_retaining_walls(request: RetainingWallsRequest):
    try:
        ...
        foundation_volume = (
            request.length * request.foundation_width * foundation_thickness
        )
        ...
        # Calculate formwork
        formwork_area = (request.length * request.height * 2) + (  # Both faces of wall
            request.length * request.foundation_width * 2
        )  # Foundation edges
...
@router.post("/api/calculate/septic_tanks", response_model=CalculationResponse)
async def calculate_septic_tanks(request: SepticTanksRequest):
    try:
        ...
        wall_concrete = 2 * (request.length * wall_thickness * request.depth) + 2 * (
            request.width * wall_thickness * request.depth
        )
        ...
        items = [
            ...
            CalculationItem(
                ...
                quantity=round(
                    excavation_volume
                    - (request.length * request.width * request.depth),
                    2,
                ),
                unit="m³",
            ),
...
@router.post("/api/calculate/swimming_pools", response_model=CalculationResponse)
async def calculate_swimming_pools(request: SwimmingPoolsRequest):
    try:
        ...
        excavation_volume = (
            (request.length + 1)
            * (request.width + 1)
            * (avg_depth + 0.5)  # Working space
        )

        # Pool concrete volumes
        floor_concrete = request.length * request.width * floor_thickness
        wall_concrete = 2 * (request.length * wall_thickness * avg_depth) + 2 * (
            request.width * wall_thickness * avg_depth
        )
...
@router.post("/api/calculate/basements", response_model=CalculationResponse)
async def calculate_basements(request: BasementsRequest):
    try:
        ...
        summary = {
            ...
            "reinforcement_weight": round(reinforcement_weight, 2),
            "waterproof_area": (
                round(waterproof_area, 2) if request.waterproofing else 0
            ),
        }
...
@router.post("/api/calculate/water_tanks", response_model=CalculationResponse)
async def calculate_water_tanks(request: WaterTanksRequest):
    try:
        ...
        if request.tank_type == "Circular":
            ...
            # Excavation
            excavation_volume = (
                math.pi * ((diameter + 1) / 2) ** 2 * (request.height + 0.5)
            )

            # Concrete volumes
            base_concrete = (
                math.pi * ((diameter / 2) + wall_thickness) ** 2 * base_thickness
            )
            wall_concrete = (
                math.pi * wall_thickness * (diameter + wall_thickness) * request.height
            )
        ...
        items = [
            ...
            CalculationItem(
                ...
                quantity=round(
                    base_concrete / base_thickness + wall_concrete / wall_thickness, 2
                ),
                unit="m²",
            ),
        ]
...
@router.post("/api/calculate/landscaping", response_model=CalculationResponse)
