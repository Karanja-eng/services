...
class EarthworksCalculator:
    ...
    def site_clearance(inputs: Dict, region: str) -> CalculationResult:
        ...
        total = (subtotal + overhead + contingency + profit) * REGION_FACTORS[region] * access_factor
    ...
    def bulk_excavation(inputs: Dict, region: str) -> CalculationResult:
        ...
        if water_table in ["Seasonal", "High"]:
            equipment.append(BreakdownItem(
        ...
        total = (subtotal + overhead + contingency + profit) * REGION_FACTORS[region] * water_factor
...
class FinishesCalculator:
    ...
    def wall_tiling(inputs: Dict, region: str) -> CalculationResult:
        ...
        # Pattern factor
        pattern_factor = {"Straight": 1.0, "Diagonal": 1.15, "Herringbone": 1.25}[pattern]
        ...
        total = (subtotal + overhead + contingency + profit) * REGION_FACTORS[region] * condition_factor
    ...
    def painting_emulsion(inputs: Dict, region: str) -> CalculationResult:
        ...
        # Color factor
        color_factor = {"White": 1.0, "Light Colors": 1.05, "Dark Colors": 1.15}[color_type]
        ...
        if primer_needed:
            materials.append(BreakdownItem(
...
class PlumbingCalculator:
    ...
    def sewer_pipe_laying(inputs: Dict, region: str) -> CalculationResult:
        ...
        pipe_rate = pipe_price_map.get(pipe_diameter, 800) if pipe_material == "PVC" else pipe_price_map.get(pipe_diameter, 800) * 1.4
    ...
    def manhole_construction(inputs: Dict, region: str) -> CalculationResult:
        ...
        exc_condition_factor = {"Dry": 1.0, "Wet": 1.3, "Rocky": 1.5}[excavation_condition]
        ...
        materials = [
            ...
            BreakdownItem(
                ...
                unit="m³",
                rate=MATERIAL_PRICES["sand_per_tonne"],
                amount=(wall_volume + base_volume) * 0.6 * MATERIAL_PRICES["sand_per_tonne"]
            ),
            BreakdownItem(
            ...
            BreakdownItem(
                ...
                amount=depth * diameter * math.pi * MATERIAL_PRICES["waterproofing_per_sqm"]
            )
        ]
...
class ConcreteCalculator:
    ...
    def mass_concrete_foundation(inputs: Dict, region: str) -> CalculationResult:
        ...
        access_factor = {"Easy": 1.0, "Moderate": 1.18, "Difficult": 1.35}[access_difficulty]
...
async def calculate_unit_rate(request: CalculationRequest):
    ...
    if request.work_type not in CALCULATORS:
        raise HTTPException(status_code=400, detail=f"Work type '{request.work_type}' not supported")
    
    calculator = CALCULATORS[request.work_type]
...
async def get_labour_rates():
    ...
    return {"rates": LABOUR_RATES, "currency": "KES", "period": "per day", "last_updated": "2025-10"}
