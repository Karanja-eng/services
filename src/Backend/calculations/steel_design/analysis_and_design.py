...
def theorem_three_moments_two_spans(
    l1: float,
    l2: float,
    I1: float,
    I2: float,
    ...
    """
    Apply theorem of three moments for two continuous spans.
    
    M_a*(l1/I1) + 2*M_b*(l1/I1 + l2/I2) + M_c*(l2/I2) = 
        -6*A1*x̄1/(I1*l1) - 6*A2*x̄2/(I2*l2)
    
    For simply supported ends: M_a = M_c = 0, solve for M_b.
    
    Args:
        l1: Length of span 1 (m)
        l2: Length of span 2 (m)
        I1: Moment of inertia of span 1 (m⁴)
        I2: Moment of inertia of span 2 (m⁴)
        A1: Area of free span bending moment diagram span 1 (Nm²)
        A2: Area of free span bending moment diagram span 2 (Nm²)
        x1_bar: Centroid distance of A1 from left support of span 1 (m)
        x2_bar: Centroid distance of A2 from right support of span 2 (m)
        E: Modulus of elasticity (N/m²)
    
    Returns:
        Tuple of (M_a, M_b) support moments (Nm)
        For simple supports: M_a = 0
    
    Reference: SK 2/18, Page 35-36
    """
    if l1 <= 0 or l2 <= 0:
    ...
    # For simply supported at ends A and C
    M_a = 0
    M_c = 0
    
    # 2*M_b*(l1/I1 + l2/I2) = -6*A1*x̄1/(I1*l1) - 6*A2*x̄2/(I2*l2)
    
    M_b = (-6 * A1 * x1_bar / (I1 * l1) - 6 * A2 * x2_bar / (I2 * l2)) / (2 * (l1/I1 + l2/I2))
...
def distribution_factor(k_i: float, k_sum: float) -> float:
    """
    Calculate distribution factor for member i at a joint.
    
    DF_i = k_i / Σk
    
    where k_i = EI/l for member i.
    
    Args:
        k_i: Stiffness factor of member i (Nm/rad)
        k_sum: Sum of stiffness factors at joint (Nm/rad)
    
    Returns:
        Distribution factor (dimensionless, 0 to 1)
    
    Reference: SK 2/20, Page 37-38
    """
    if k_sum <= 0:
        raise ValueError("Sum of stiffness factors must be positive")
    if k_i < 0:
        raise ValueError("Stiffness factor cannot be negative")
