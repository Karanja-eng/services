...
# MODELS
# ============================================================================
# Models moved to models.py for modularity
...
@router.post("/api/process", response_model=BuildingModel)
async def process(
    file_id: str = Form(...),
    ...
    try:
        ...
        if use_yolo and detector:
            ...
            doors = det_data["doors"] or doors
            windows = det_data["windows"] or windows
            rooms = det_data["rooms"] or rooms
            stairs = det_data["stairs"]
            columns = det_data["columns"]
            furniture = det_data["furniture"]
            railings = det_data["railings"]
            electrical = det_data["electrical"]
            plumbing = det_data["plumbing"]
            
            # Application of Engineering Rules
            db_pt = electrical[0].position if electrical else [0.5, 0.5]
            rule_points = engine.auto_place_electrical(furniture, walls)
