...
class EurocodeStairInput(BaseModel):
    ...
    waist_thickness: int = Field(..., ge=100, le=500, description="Waist thickness in mm")
    ...
    # Loading
    live_load: float = Field(..., ge=0, le=20, description="Characteristic live load in kN/m²")
    ...
    def validate_cantilever_type(cls, v, values):
        if 'stair_type' in values and values['stair_type'] == StairType.CANTILEVER and v is None:
...
class EurocodeStairCalculator:
    ...
    def __init__(self, input_data: EurocodeStairInput):
        ...
        self.concrete = EurocodeTablesEC2.CONCRETE_PROPERTIES[input_data.concrete_class.value]
        self.steel = EurocodeTablesEC2.STEEL_PROPERTIES[input_data.steel_grade.value]
        self.warnings = []
        
        # Design strengths (Clause 3.1.6 and 3.2.7)
        self.fcd = (EurocodeTablesEC2.ALPHA_CC * self.concrete["fck"]) / EurocodeTablesEC2.GAMMA_C
    ...
    def calculate_loading(self) -> EurocodeLoadingResults:
        ...
        self_weight_per_m2 = waist_m * EurocodeTablesEC2.UNIT_WEIGHT_CONCRETE * inclined_multiplier
    ...
    def design_flexural_reinforcement(self, forces: EurocodeDesignForces) -> EurocodeSteelDesign:
        """Design flexural reinforcement per EN 1992-1-1 Clause 6.1"""
        
        # Geometry
        assumed_bar_dia = 12
        effective_depth = self.input.waist_thickness - self.input.cover - (assumed_bar_dia / 2)
        ...
        if mu_ed > mu_lim:
            self.warnings.append("Normalized moment exceeds limit. Compression reinforcement required or increase depth.")
            mu_ed = mu_lim
        ...
        # Calculate spacing
        bar_spacing = int((b - 2 * self.input.cover) / (num_bars - 1)) if num_bars > 1 else int(b / 2)
        ...
        As_dist = max(0.2 * As_actual, EurocodeTablesEC2.MIN_STEEL_RATIO * self.input.waist_thickness * 1000)
        dist_bar_dia = 8
    ...
    def design_shear(self, forces: EurocodeDesignForces, steel: EurocodeSteelDesign) -> EurocodeShearDesign:
        ...
        v_rd_max = alpha_cw * b * 0.9 * d * nu * self.fcd / (1 / math.tan(math.radians(45)) + math.tan(math.radians(45)))
        ...
        if not shear_adequate:
            self.warnings.append("Shear stress exceeds maximum capacity. Increase section depth.")
    ...
    def perform_design_checks(self, steel: EurocodeSteelDesign, shear: EurocodeShearDesign) -> EurocodeDesignChecks:
        ...
        if rho <= rho_0:
            factor = min(11 + 1.5 * math.sqrt(self.concrete["fck"] / rho_0) + 3.2 * math.sqrt(self.concrete["fck"] / rho_0) * ((rho_0 / rho) - 1) ** 1.5, 1.5)
        else:
        ...
        # Cover check
        min_cover = EurocodeTablesEC2.COVER_REQUIREMENTS[self.input.exposure_class.value]
        cover_ok = self.input.cover >= min_cover
        
        # Crack width check (simplified - assume OK if spacing is OK)
        crack_width_ok = spacing_ok
        
        all_passed = all([deflection_ok, spacing_ok, min_steel_ok, shear_ok, cover_ok, crack_width_ok])
    ...
    def perform_complete_design(self) -> EurocodeDesignOutput:
        ...
        status = "DESIGN SATISFACTORY - EN 1992-1-1:2004" if checks.all_checks_passed else "DESIGN REQUIRES REVISION"
        
        input_summary = {
            "stair_type": self.input.stair_type.value,
            "cantilever_type": self.input.cantilever_type.value if self.input.cantilever_type else None,
...
async def design_eurocode_stair(input_data: EurocodeStairInput):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Eurocode calculation error: {str(e)}")
...
class BS8110Calculator:
    ...
    BAR_AREAS = {
        8: 50.3, 10: 78.5, 12: 113.1, 16: 201.1, 20: 314.2, 25: 490.9, 32: 804.2
    }
    ...
    def calculate_loading(self):
        pitch_angle = math.atan(self.input.riser_height / self.input.tread_length)
        inclined_multiplier = math.sqrt(1 + (self.input.riser_height / self.input.tread_length) ** 2)
    ...
    def design_flexural_reinforcement(self, forces):
        ...
        z = min(effective_depth * (0.5 + math.sqrt(0.25 - K / 0.9)), 0.95 * effective_depth)
        ...
        bar_spacing = int((width_mm - 2 * self.input.cover) / (num_bars - 1)) if num_bars > 1 else int(width_mm / 2)
    ...
    def perform_complete_design(self) -> BS8110DesignOutput:
        ...
        status = "DESIGN SATISFACTORY - BS 8110-1:1997" if checks["all_checks_passed"] else "DESIGN REQUIRES REVISION"
...
async def design_bs8110_stair(input_data: BS8110StairInput):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"BS 8110 calculation error: {str(e)}")
