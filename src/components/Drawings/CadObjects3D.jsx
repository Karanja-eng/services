...
const SceneObject = ({ obj }) => {
    ...
    React.useEffect(() => {
        if (ref.current && obj.metadata) {
            layerController.registerNode(ref.current, obj.metadata);
            return () => layerController.unregisterNode(ref.current);
        }
    }, [obj.metadata]);
    ...
    switch (obj.type) {
        ...
        case "circle":
            return (
                <line key={obj.id} ref={ref} position={[obj.center.x, obj.center.y, obj.center.z || 0]}>
        ...
        case "rectangle":
            ...
            return (
                <line key={obj.id} ref={ref} position={[(obj.start.x + obj.end.x) / 2, (obj.start.y + obj.end.y) / 2, 0]}>
                    <edgesGeometry attach="edges">
        ...
        case "box":
            return (
                <mesh key={obj.id} ref={ref} position={[obj.position.x, obj.position.y, obj.position.z || 0]}>
