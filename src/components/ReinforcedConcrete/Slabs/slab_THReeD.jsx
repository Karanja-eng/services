...
export function DrawSlabMS2({
  ...
  return (
    <group name="MS2-Slab-External-Support">
      ...
      {showLabels && <group name="labels">{/* Add text labels here */}</group>}
    </group>
  );
}
...
export function DrawSlabMS3({
  ...
  const needBobs = detailType === "A" && slabDepth >= 0.15;
  ...
  const createBobbedBarPath = () => {
    ...
    path.add(
      new THREE.LineCurve3(
        ...
        new THREE.Vector3(
          horizontalLength + bendRadius * 2,
          bendRadius + bobLength,
          0
        )
      )
    );
    ...
    <group name="MS3-Slab-Unrestrained-Support">
      ...
      {showRebar && (
        <group name="reinforcement">
          ...
          {(detailType === "B" || detailType === "C") && slabDepth < 0.15 && (
            <>
              {["left", "right"].map((side) =>
                Array.from({ length: Math.floor(numMainBars / 2) }).map(
                  (_, i) => {
      ...
      {/* Labels */}
      {showLabels && <group name="labels">{/* Detail type indicator */}</group>}
    </group>
  );
}
...
export function DrawSlabMS4({
  ...
  return (
    <group name="MS4-Cantilever-Slab">
        ...
        <group name="reinforcement">
          ...
          <group name="bottom-bars">
            {Array.from({ length: numBottomBars }).map((_, i) => {
              ...
              const bottomBarLength = Math.min(
                cantileverLength * 0.5,
                backspan * 0.3
              );
            ...
            {Array.from({ length: Math.floor(numTopBars / 3) }).map((_, i) => {
              ...
              return (
                <mesh
                  ...
                  material={topBarMaterial}
                  position={[
                    cantileverLength - 0.5 * tensionLap,
                    slabDepth / 2 - cover,
                    z,
                  ]}
                />
      ...
      {/* Labels */}
      {showLabels && (
        <group name="labels">{/* Cantilever length indicator */}</group>
      )}
    </group>
  );
}
...
export function DrawSlabMS5({
  ...
  columnStripBarSpacing = 0.15,
  middleStripBarSpacing = 0.25,
  colors = {
    concrete: "#a8a8a8",
    mainRebar: "#cc3333",
    distributionBars: "#cc8833",
  ...
  const numColumnStripBars =
    Math.floor(columnStripWidth / columnStripBarSpacing) + 1;
  const numMiddleStripBars =
    Math.floor(middleStripWidth / middleStripBarSpacing) + 1;
              ...
              [-columnWidth / 4, columnWidth / 4].map((offset, idx) => {
                ...
                return (
                  <mesh
                    ...
                    position={[
                      colX + offset,
                      -slabDepth / 2 + cover + columnStripBarDiameter / 2,
                      0,
                    ]}
                    rotation={[Math.PI / 2, 0, 0]}
                  />
...
export function DrawSlabMS6({
  ...
  const effectiveDepth = slabDepth - cover - 0.02; // Approximate
  const firstPerimeterDistance = 2 * effectiveDepth; // 2d from column face

  const slabSize =
    columnWidth +
    firstPerimeterDistance * 2 +
    linkSpacing * perimeters * 2 +
    1.0;
  ...
  return (
    <group name="MS6-Flat-Slab-Shear-Reinforcement">
      ...
      {showRebar && (
        <group name="shear-reinforcement">
          ...
          {Array.from({ length: perimeters }).map((_, perimeterIdx) => {
            const distance =
              firstPerimeterDistance + perimeterIdx * linkSpacing;
            const perimeterSize = columnWidth + 2 * distance;
            const numLinksPerSide =
              Math.floor(perimeterSize / (effectiveDepth * 1.5)) + 1;

            const perimeterPath = createLinkPerimeter(perimeterSize);
            const perimeterPoints = perimeterPath.getPoints(
              numLinksPerSide * 4
            );

            return (
              <group
                key={`perimeter-${perimeterIdx}`}
                name={`perimeter-${perimeterIdx + 1}`}
              >
                {/* Perimeter outline */}
                <lineSegments>
                  <bufferGeometry attach="geometry">
                    <bufferAttribute
                      ...
                      array={
                        new Float32Array(
                          perimeterPoints.flatMap((p) => [p.x, p.y, p.z])
                        )
                      }
                      itemSize={3}
                    />
                ...
                {perimeterPoints
                  .filter((_, idx) => idx % Math.floor(numLinksPerSide) === 0)
                  .map((point, linkIdx) => {
                    const verticalLinkPath = createVerticalLink(
                      slabDepth - 2 * cover
                    );
          ...
          <group name="hanger-bars">
            {[...Array(4)].map((_, i) => {
              ...
              const radius =
                columnWidth / 2 +
                firstPerimeterDistance +
                ((perimeters - 1) * linkSpacing) / 2;
              ...
              const rotationAxis =
                i % 2 === 0 ? [0, 0, Math.PI / 2] : [Math.PI / 2, 0, 0];
              ...
              return (
                <mesh
                  ...
                  position={[
                    x,
                    slabDepth / 2 - cover - hangerBarDiameter / 2,
                    z,
                  ]}
      ...
      {/* Labels */}
      {showLabels && <group name="labels">{/* Spacing annotations */}</group>}
    </group>
  );
}

// ============================================================================
// MS7: FLAT SLABS - COLUMN DROPS
// ============================================================================

/**
 * MS7: Flat Slabs - Column Drops
 * Shows column drop details with:
 * - Main bottom bars carried through drop
 * - Nominal reinforcement in bottom of drop
 * - 12mm bars at 300mm pitch minimum
 */
export function DrawSlabMS7({
  slabDepth = 0.25,
  dropDepth = 0.4,
  dropSize = 2.0,
  columnWidth = 0.4,
  ...
  const dropThickness = dropDepth - slabDepth;
  ...
  const dropGeometry = useMemo(
    () => new THREE.BoxGeometry(dropSize, dropThickness, dropSize),
    [dropSize, dropThickness]
  );
  ...
  const nominalBarMaterial = useMemo(
    () =>
      new THREE.MeshStandardMaterial({
        color: colors.nominalRebar,
        metalness: 0.7,
        roughness: 0.3,
        wireframe: wireframe,
      }),
    [colors.nominalRebar, wireframe]
  );

  const numMainBars = Math.floor(slabSize / mainBarSpacing) + 1;
  const numNominalBarsInDrop = Math.floor(dropSize / nominalBarSpacing) + 1;

  return (
    <group name="MS7-Flat-Slab-Column-Drop">
      {/* Concrete Slab */}
      {showConcrete && (
        <mesh
          geometry={slabGeometry}
      ...
      {showConcrete && (
        <mesh
          ...
          position={[0, -slabDepth / 2 - dropThickness / 2, 0]}
        />
      )}
      ...
      {showRebar && (
        <group name="reinforcement">
          {/* Main bottom bars carried through drop - Direction 1 */}
          <group name="main-bottom-bars-x">
            {Array.from({ length: numMainBars }).map((_, i) => {
              const z = -slabSize / 2 + i * mainBarSpacing;
              const y = -slabDepth / 2 + cover + mainBarDiameter / 2;
          ...
          {/* Main bottom bars - Direction 2 (perpendicular) */}
          <group name="main-bottom-bars-z">
            {Array.from({ length: numMainBars }).map((_, i) => {
              const x = -slabSize / 2 + i * mainBarSpacing;
              const y = -slabDepth / 2 + cover + mainBarDiameter * 1.5;
          ...
          {/* Nominal reinforcement in bottom of drop - 12mm @ 300 - Direction 1 */}
          <group name="nominal-drop-bars-x">
            {Array.from({ length: numNominalBarsInDrop }).map((_, i) => {
              const z = -dropSize / 2 + i * nominalBarSpacing;
              const y = -dropDepth / 2 + cover + nominalBarDiameter / 2;
          ...
          {/* Nominal reinforcement in bottom of drop - Direction 2 */}
          <group name="nominal-drop-bars-z">
            {Array.from({ length: numNominalBarsInDrop }).map((_, i) => {
              const x = -dropSize / 2 + i * nominalBarSpacing;
              const y = -dropDepth / 2 + cover + nominalBarDiameter * 1.5;
          ...
          {/* Top reinforcement over drop (column strip bars) */}
          <group name="top-drop-bars">
            {Array.from({ length: Math.floor(numNominalBarsInDrop * 0.7) }).map(
              (_, i) => {
                const z = -dropSize / 2 + i * (nominalBarSpacing * 1.5);
                const y = slabDepth / 2 - cover - mainBarDiameter / 2;
                const topBarLength = dropSize * 1.5; // Extend beyond drop
          ...
          {/* 75mm minimum edge distance indicators (visual markers) */}
          {showLabels && (
            <group name="edge-clearance-markers">
              {/* Corner markers for 75mm clearance requirement */}
              {[
      ...
      {showLabels && (
        <group name="labels">
          {/* Drop dimensions: 75mm min clearance note */}
          {/* Bar specifications: 12mm @ 300 */}
        </group>
      )}
    </group>
  );
        ...
        <group name="concrete-structure">
          ...
          {(detailType === "B" || detailType === "C") && (
            <group name="perpendicular-ribs">
              {Array.from({ length: Math.floor(span / ribSpacing) }).map(
                (_, i) => {
                  const xPos = -span / 2 + (i + 0.5) * ribSpacing;

                  const perpRibGeometry = new THREE.BoxGeometry(
                    ribWidth,
                    ribDepth,
                    width
                  );
      ...
      {showRebar && (
        <group name="reinforcement">
          ...
          <group name="top-fabric">
            {/* Main wires */}
            {Array.from({ length: Math.floor(width / 0.2) + 1 }).map((_, i) => {
              const z = -width / 2 + i * 0.2;
            ...
            {/* Cross wires */}
            {Array.from({ length: Math.floor(span / 0.2) + 1 }).map((_, i) => {
              const x = -span / 2 + i * 0.2;
                  ...
                  {ribBarCount === 2 ? (
                    <>
                      <mesh
                        ...
                        position={[
                          0,
                          -ribDepth / 2 + cover + ribBarDiameter / 2,
                          zPos - ribWidth / 4,
                        ]}
                        rotation={[0, 0, Math.PI / 2]}
                      />
          ...
          {ribDepth > 0.75 && (
            <group name="lacing-bars">
              {Array.from({ length: numRibs }).map((_, ribIdx) => {
                const zPos = -width / 2 + (ribIdx + 0.5) * actualRibSpacing;
                const numLacers = Math.floor(ribDepth / 0.3); // Typically every 300mm
                ...
                return (
                  <group key={`lacer-group-${ribIdx}`}>
                    {Array.from({ length: numLacers }).map((_, lacerIdx) => {
                      const yPos =
                        -ribDepth / 2 +
                        (lacerIdx + 1) * (ribDepth / (numLacers + 1));
