...
export function RoofScene({ roofConfig, settings = {} }) {
    // Map settings to visibility for compatibility with existing logic
    const visibility = {
        rafters: settings.rafters !== false,
        purlins: settings.purlins !== false,
        wallPlates: settings.wallPlates !== false,
        covering: settings.covering !== false,
        ridge: settings.ridge !== false,
        fascia: settings.fascia !== false,
        rainwaterGoods: settings.rainwaterGoods !== false,
    };

    // Fallback if config isn't passed correctly (though it should be)
