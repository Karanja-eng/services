...
const ApproximateQuantities = ({
  ...
  const materialPrices = React.useMemo(
    () => ({
      concrete: {
        cement: {
          label: "Cement (per 50kg bag)",
          unit: "bag",
          defaultPrice: 850,
          ratio: 7,
        },
        sand: {
          label: "Sand (per tonne)",
          unit: "t",
          defaultPrice: 2500,
          ratio: 0.5,
        },
        aggregates: {
          label: "Aggregates (per tonne)",
          unit: "t",
          defaultPrice: 3200,
          ratio: 0.8,
        },
      ...
      excavation: {
        ...
        machinery: {
          label: "Machinery hire (per hour)",
          unit: "hr",
          defaultPrice: 15000,
          ratio: 0.1,
        },
        operator: {
          label: "Operator (per day)",
          unit: "day",
          defaultPrice: 2500,
          ratio: 0.1,
        },
        transport: {
          label: "Transport (per m³)",
          unit: "m³",
          defaultPrice: 300,
          ratio: 1,
        },
      ...
      piling: {
        concrete: {
          label: "Concrete C25/30 (per m³)",
          unit: "m³",
          defaultPrice: 12000,
          ratio: 1,
        },
        steel_casing: {
          label: "Steel casing (per m)",
          unit: "m",
          defaultPrice: 2500,
          ratio: 1,
        },
        drilling: {
          label: "Drilling (per m)",
          unit: "m",
          defaultPrice: 3500,
          ratio: 1,
        },
        reinforcement: {
          label: "Reinforcement (per m)",
          unit: "m",
          defaultPrice: 850,
          ratio: 1,
        },
      ...
      pipes: {
        pipe_material: {
          label: "Pipe material (per m)",
          unit: "m",
          defaultPrice: 1200,
          ratio: 1,
        },
        fittings: {
          label: "Fittings (per joint)",
          unit: "joint",
          defaultPrice: 450,
          ratio: 0.1,
        },
        bedding: {
          label: "Bedding material (per m)",
          unit: "m",
          defaultPrice: 300,
          ratio: 1,
        },
      ...
      roads: {
        aggregate: {
          label: "Aggregate material (per m³)",
          unit: "m³",
          defaultPrice: 3200,
          ratio: 1,
        },
  ...
  const calculateMaterialQuantity = React.useCallback(
    (baseQuantity, material, itemType) => {
        ...
        if (materialPrices[itemType]) {
          Object.keys(materialPrices[itemType]).forEach((material) => {
            ...
            if (!initialPrices[key]) {
              initialPrices[key] =
                materialPrices[itemType][material].defaultPrice;
            }
          });
        }
      });
  ...
  const calculateApproximateCosts = React.useCallback(() => {
  ...
  }, [priceInputs, takeoffData, calculateApproximateCosts]);
      ...
      <div className="bg-white rounded-lg shadow-sm border p-8">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-6">
            Please complete the quantity take-off first.
          </p>
  ...
  const totalProjectCost = calculatedCosts.reduce(
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
          <h2 className="text-xl font-bold text-gray-800">
            Approximate Quantities & Costing
          </h2>
            ...
            >
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600">
                  Quantity: {item.quantity.toFixed(2)} {item.unit} | Type:{" "}
                  {itemType}
                </p>
              </div>
                      ...
                      <div className="flex flex-col space-y-1">
                        <div className="flex space-x-2">
                          <input
                            type="number"
                            value={
                              priceInputs[inputKey] || material.defaultPrice
                            }
                            onChange={(e) =>
                              handlePriceChange(inputKey, e.target.value)
                            }
                        ...
                        <div className="text-sm">
                          <span className="text-gray-600">
                            Quantity: {materialQuantity.toFixed(2)}{" "}
                            {material.unit}
                          </span>
              ...
              {calculatedCosts.find((cost) => cost.id === item.id) && (
                <div className="mt-4 p-3 bg-blue-50 rounded-md">
                  <p className="text-blue-800 font-medium">
                    Total Item Cost: KES{" "}
                    {calculatedCosts
                      .find((cost) => cost.id === item.id)
...
export default ApproximateQuantities;
